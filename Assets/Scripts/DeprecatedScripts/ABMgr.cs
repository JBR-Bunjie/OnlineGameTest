...
public class ABMgr : MonoSingleton<ABMgr> {
    ...
    // 获取包体统一前缀路径
    public string StreamingAssetsPath => Application.streamingAssetsPath + "/"; // Unity官方的统一加载包体的前缀
    ...
    private void LoadABMain() {
        // 同步加载主包
        abMain = AssetBundle.LoadFromFile(StreamingAssetsPath + ABMainName);
    ...
    private void LoadABDependencies(string abName) {
        ...
        for (int i = 0; i < abDependencies.Length; i++) {
            if (!abDic.ContainsKey(abDependencies[i])) {
                ab = AssetBundle.LoadFromFile(StreamingAssetsPath + abDependencies[i]);
    ...
    public T LoadRes<T>(string abName, string resName, UnityAction<T> action = null) where T : Object {
        ...
        if (!abDic.ContainsKey(abName)) {
            ab = AssetBundle.LoadFromFile(StreamingAssetsPath + abName);
            abDic.Add(abName, ab);
        }
        else
            ab = abDic[abName];
    ...
    /// <param name="callback">回调函数</param>
    /// <returns></returns>
    private IEnumerator Ie_LoadAsyncRes<T>(string abName, string resName, UnityAction<T> callback = null)
        where T : Object {
        yield return null;
        LoadABDependencies(abName);
        ...
        if (!abDic.ContainsKey(abName)) {
            do {
                abCReq = AssetBundle.LoadFromFileAsync(StreamingAssetsPath + abName);
        ...
        if (callback != null) {
            if (abReq.asset == null) {
                Debug.Log(StreamingAssetsPath + abName + "/" + resName + "is Null");
            }
            else {
    ...
    /// <param name="callback">回调函数</param>
    /// <returns></returns>
    private IEnumerator Ie_LoadAsyncRes(string abName, string resName, System.Type type,
