...
namespace NetworkProcessor {
    ...
    /// unityWebRequest.downloadHandler = new DownloadHandlerForResumingTransfer("文件保存的路径", unityWebRequest);
    /// unityWebRequest.SendWebRequest();
    /// </summary>
    public class DownloadHandlerForResumingTransfer : DownloadHandlerScript {
        /// <summary>
        /// 文件正式开始下载事件,此事件触发以后即可获取到文件的总大小
        /// </summary>
        public event Action StartDownloadEvent;
        ...
        /// 下载速度,单位:KB/S 保留两位小数
        /// </summary>
        public float Speed => ((int)(DownloadSpeed / 1024 * 100)) / 100.0f;

        /// <summary>
        /// 文件的总大小
        /// </summary>
        public long FileSize => TotalFileSize;
        ...
        public DownloadHandlerForResumingTransfer(string path, UnityWebRequest request) : base(new byte[1024 * 1024]) {
            Path = path;
        ...
        ~DownloadHandlerForResumingTransfer() {
            Clean();
        }
